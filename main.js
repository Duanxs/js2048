...
function prepareForMobile() {
    ...
    $("#grid-container").css("width", gridConatainerWidth - 2*cellSpace);
    $("#grid-container").css("height", gridConatainerWidth - 2*cellSpace );
    $("#grid-container").css("padding", cellSpace);
    $("#grid-container").css("border-radius", 0.02*gridConatainerWidth);

    $(".grid-cell").css("width", cellSideLength);
    $(".grid-cell").css("height", cellSideLength);
    $(".grid-cell").css("border-radius", 0.02*gridConatainerWidth);
}
                ...
                if (board[i][j] != 0 ) {
                    for(var k = 0; k < i; k ++ ) {
                        if(board[k][j] == 0 && noBlockVertical(k,i,j,board)) {
                            ...
                            board[k][j] = board[i][j];
                            board[i][j] = 0;
                        } else if (board[k][j] == board[i][j] && noBlockVertical(k,i,j,board) && !hasMixed[i][k]) {
                            showMoveAnimation(i,j,k,j);
                            board[k][j] += board[i][j];
                            board[i][j] = 0;
                            score += board[k][j];
                ...
                if (board[i][j] != 0 ) {
                    for(var k = 3; k > i; k --) {
                        if (board[k][j] == 0 && noBlockVertical(i,k,j,board)) {
                            ...
                            board[k][j] = board[i][j];
                            board[i][j] = 0;
                        } else if (board[k][j] == board[i][j] && noBlockVertical(i,k,j,board) && !hasMixed[i][k]) {
                            showMoveAnimation(i,j,k,j);
                            board[k][j] += board[i][j];
                            board[i][j] = 0;
                            score += board[k][j];
